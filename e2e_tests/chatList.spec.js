...
test.describe("chatList tests", () => {
  ...
  test("handles pagination with scrolling", async ({ page }) => {
    ...
    await expect(
      page.getByTestId("chatListBtn").nth(initialChatCount),
    ).toBeVisible()
  })
  ...
  test("opens chat with user on click", async ({ page }) => {
    ...
    expect(await page.getByTestId("chatUserName").textContent()).toMatch(
      await chatList.nth(0).textContent(),
    )
    ...
    await page.waitForTimeout(200)
    expect(await page.getByTestId("chatUserName").textContent()).toMatch(
      await chatList.nth(5).textContent(),
    )
  })
  ...
  test.describe("notification updates", () => {
    test.beforeEach(async ({ page }) => {
      await expect(
        page.getByRole("img", { name: "message notification" }),
      ).toHaveCount(0)
    ...
    test("displays notification icon when logged user receives a message", async ({
      page,
    }) => {
      await expect(
        page.getByRole("img", { name: "message notification" }),
    ...
    test("hides notification message when user opens the chat", async ({
      page,
    }) => {
      await expect(
        page.getByRole("img", { name: "message notification" }),
    ...
    test("if chat is already open, hides notification message when receiver sends a message", async ({
      page,
    }) => {
      await expect(
        page.getByRole("img", { name: "message notification" }),
  ...
  test.describe("search bar display", () => {
    ...
    test("shows search bar when button is clicked on smaller screens", async ({
      page,
    }) => {
      page.setViewportSize({ width: 1300, height: 900 })
      ...
      expect(
        page.getByRole("textbox", { name: "Search for a fellow hiker" }).nth(1),
      ).toBeVisible()
    })
    ...
    test("hides search bar when focus is lost", async ({ page }) => {
      ...
      const searchBar = page
        .getByRole("textbox", { name: "Search for a fellow hiker" })
        .nth(1)
