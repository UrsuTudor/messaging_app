...
test.describe("searchbar tests", () => {
  ...
  test.describe("filtering logic", () => {
    test.describe("on wider screens", () => {
      ...
      test("filters the event list", async ({ page, request }) => {
        await request.delete("/api/v1/test/delete_events")
        await request.post("/api/v1/test/future_events")

        let input = page
          .getByRole("textbox", { name: "Search by title or location" })
          .nth(0)
        await input.fill("Test Event 24")
        await expect(page.getByText("Test Event 24")).toHaveCount(1)
      })
    })
  })
})
