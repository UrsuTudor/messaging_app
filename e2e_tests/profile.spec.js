...
test.describe("profile tests", () => {
  ...
  test.describe("allows the logged user to edit their profile", () => {
    test("changes description", async ({ page }) => {
      ...
      const updateDescrBtn = page.getByRole("button", {
    ...
    test("changes avatar", async ({ page }) => {
      ...
      await page
        ...
        .setInputFiles(path.join(process.cwd(), "public", "icon.png"))
  ...
  test("displays future and past events", async ({ request, page }) => {
    await request.delete("/api/v1/test/delete_events")
    await request.post("/api/v1/test/future_event")
    await request.post("/api/v1/test/past_event")
    await page.reload()
  ...
  test("opens new chat with user", async ({ page }) => {
    ...
    await page.getByTestId("userListBtn").first().click()
